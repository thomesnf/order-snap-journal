...
if [ "$HEALTH_CHECK" != "200" ]; then
  ...
  # Check for crash/panic in logs
  echo "Checking GoTrue logs for crashes..."
  GOTRUE_CRASH=$(docker logs supabase-auth --tail=20 2>&1 | grep -i "panic\|segmentation\|fatal" || echo "")
  
  if [ -n "$GOTRUE_CRASH" ]; then
    echo "⚠️  GoTrue is CRASHING on startup!"
    echo ""
    echo "Common causes and fixes:"
    echo "  1. JWT_SECRET format issue:"
    echo "     - Must be at least 32 characters"
    echo "     - Should be base64 encoded (run: openssl rand -base64 32)"
    echo "     - Check .env.self-hosted: JWT_SECRET=..."
    echo ""
    echo "  2. Database connection string mismatch:"
    echo "     - Verify POSTGRES_PASSWORD matches between .env.self-hosted and docker-compose"
    echo "     - Check DATABASE_URL in docker-compose.self-hosted.yml"
    echo ""
    echo "  3. Missing required environment variables:"
    echo "     - SITE_URL, API_EXTERNAL_URL, GOTRUE_SITE_URL"
    echo ""
    echo "To view full crash details:"
    echo "  docker logs supabase-auth --tail=100"
    echo ""
    echo "To fix:"
    echo "  1. Update .env.self-hosted with correct values"
    echo "  2. Restart services: docker-compose -f docker-compose.self-hosted.yml restart"
    ...
    echo "  - Verify JWT_SECRET is set correctly (32+ characters, base64 encoded)"
